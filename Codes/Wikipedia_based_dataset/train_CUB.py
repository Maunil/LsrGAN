...
from models import _netD, _netG, _param, _regressor, _seen_vs_unseen
import time 
...
def eval_fakefeat_test(it, netG, netD, dataset, param, result):
    ...
    for i in range(dataset.test_cls_num):
        text_feat = np.tile(dataset.test_text_feature[i].astype('float32'), (opt.nSample, 1)) # This will duplicate the text feature as per the number of examples we want 
    ...
    '''
    dataset.pfc_feat_data_test has only 2933 images, that is less than the generated image features.
    '''
